...
const SHORTS_PACKAGES = [
  { qty: 1, label: "1 Short Video Ad", price: 200, featured: false },
  { qty: 5, label: "5 Short Video Ads", price: 900, featured: true },
  { qty: 10, label: "10 Short Video Ads", price: 1600, featured: false },
];

const FORMATS = ["YouTube Shorts", "TikTok", "Reels"];

export function Shorts() {
  const tg = "https://t.me/crypoclicksio";
    ...
    <section className="mx-auto max-w-7xl px-6 pb-28">
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {FORMATS.map((f) => (
          <span
            key={f}
            className="inline-flex items-center rounded-full border border-border bg-card px-3 py-1 text-xs font-semibold text-muted-foreground"
          >
            {f}
          </span>
        ))}
        ...
        {SHORTS_PACKAGES.map((pkg) => (
          ...
          >
            ...
            <div className="mt-6 flex items-baseline gap-1.5">
              <span className="font-black text-5xl tracking-tighter">${pkg.price}</span>
            </div>
            {pkg.qty > 1 && (
              <div className="mt-1.5 text-xs text-muted-foreground">
                ~${Math.round(pkg.price / pkg.qty)}/video
              </div>
            )}
            <a
              href={tg}
              target="_blank"
              rel="noreferrer"
              className="mt-6 flex w-full items-center justify-center gap-2 rounded-lg border border-border bg-transparent px-4 h-10 text-sm font-semibold text-foreground hover:border-primary hover:text-primary transition-colors"
            >
              <Send className="h-3.5 w-3.5" /> Order via Telegram
            </a>
          </div>
        ))}
      </div>
